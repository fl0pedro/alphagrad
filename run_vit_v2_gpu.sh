#!/bin/bash
#SBATCH --job-name=vit_v2
#SBATCH --time=24:00:00
#SBATCH --output=/Users/assmuth/dsnn/vit_v2_%j.out
#SBATCH --partition=pgi15
#SBATCH --nodes=1
#SBATCH --nodelist=pgi15-gpu12
#SBATCH --gres=gpu:nvidia_geforce_rtx_4090:4
...
NAME="vit_v2${VARIANT:+_$VARIANT}$([ "$VALUE_NORM" = popart ] && echo _popart)_s${SEED}_ss${MAX_SUBSTEPS}"
...
( cd ~/dsnn && uv run --no-sync ray start --head --node-ip-address=$HEAD_IP \
    --port=$RAY_PORT --num-gpus=${NUM_GPUS:-4} --num-cpus=64 --block ) &
disown
sleep 25
