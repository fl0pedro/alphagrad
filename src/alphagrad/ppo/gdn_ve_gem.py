...
def main():
    ...
    def start_async_rollouts(
        ...
        # Ensure NUM_ENVS is cleanly divisible by num_threads
        assert num_envs % num_threads == 0
        chunk_size = num_envs // num_threads

        traj_queue = Queue(maxsize=2)
        weight_queue = Queue(maxsize=1)

        # Pre-slice masks for thread distribution
        chunk_masks = init_masks.reshape(num_threads, chunk_size, -1)
            ...
            with ThreadPoolExecutor(max_workers=num_threads) as pool:
                for _ in range(num_episodes):
                    if not weight_queue.empty():
                        current_agent = weight_queue.get()

                    key, *subkeys = jrand.split(key, num_threads + 1)

                    def run_chunk(i):
                        chunk_keys = jrand.split(subkeys[i], chunk_size)

                        # Reset just this thread's chunk
                        state_chunk = jax.vmap(lambda _: env.reset())(
                            jnp.arange(chunk_size)
                        )

                        # Rollout chunk (XLA compiles this for chunk_size and executes concurrently)
                        _, trajs = rollout_fn(
                            current_agent,
                            rollout_length,
                            state_chunk,
                    ...
                    # Dispatch chunks to freethreaded CPU cores
                    traj_chunks = list(pool.map(run_chunk, range(num_threads)))

                    # Merge the SoA PyTrees back to shape (NUM_ENVS, ...)
                    trajectories = jax.tree_util.tree_map(
                        lambda *leaves: jnp.concatenate(leaves, axis=0), *traj_chunks
                    )

                    traj_queue.put(trajectories)

        threading.Thread(target=rollout_worker, daemon=True).start()
        return traj_queue, weight_queue
    ...
    # Hoist mask generation outside the loop
    init_mask = jnp.zeros(total_v, dtype=jnp.float32).at[valid_vertices - 1].set(1.0)
    init_masks = jnp.broadcast_to(init_mask, (NUM_ENVS, total_v))

    worker_key, key = jrand.split(key)

    # Start the async worker
    traj_queue, weight_queue = start_async_rollouts(
        agent=agent,
        rollout_length=ROLLOUT_LENGTH,
        init_masks=init_masks,
        num_episodes=EPISODES,
        worker_key=worker_key,
        num_envs=NUM_ENVS,  # Forces the split to be exactly 64
    )
    ...
    for episode in pbar:
        ...
        # 2. Train loop (GPU bound)
        for i in range(MINIBATCHES):
            train_key, key = jrand.split(key)
            # JAX dispatches these to GPU immediately and returns futures.
            # The Python loop does NOT wait for the GPU to finish.
            agent, opt_state, metrics = train_agent(
                agent, opt_state, batches[i], train_key
            )

        # 3. Push updated weights to worker for the NEXT rollout.
        # Optional: jax.block_until_ready(agent) to prevent the CPU worker from
        # queueing XLA transfers before the GPU is done computing them.
        if weight_queue.empty():
            weight_queue.put(agent)
