...
import optax
import wandb
from alphagrad.vertexgame.vertex_game_w_tokens import print_thread_metrics
...
def main():
    ...
    def train_step(agent, opt_state, data, key):
        def loss_fn(agent):
            ...
            output = agent(obs_flat, keys)  # (B*Rollout, 1+num_v)
            v_preds = output[:, 0]
            policy_logits = output[:, 1:]

            targets_flat = data["policy"].reshape(-1, num_v)
    ...
    dummy_axes = jnp.arange(args.num_envs)
