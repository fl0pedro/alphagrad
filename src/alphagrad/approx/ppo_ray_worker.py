...
class PPORayWorker:
    ...
    def __init__(self, args_dict: dict, seed: int = 0, cpu_workers: list | None = None):
        ...
        # the rollout worker's env operates on the SAME scalar-loss graph as the
        # CPU measure-actors (else the policy's vertex/action space wouldn't
        # match the graph the pool eliminates on). Shared wrap — see
        # ``maybe_scalar_loss``.
