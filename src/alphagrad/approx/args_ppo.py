"""JAX-free CLI arguments specific to `ppo_ray.py`.
...
This module holds the PPO-ONLY half of that surface: the flags az
shares (`--seed`, `--dataset`, `--lr`, `--latency-inner-reps`) live in
`args_common.add_common_args`, with PPO's values for them in
`COMMON_DEFAULTS` below. `ppo_args.make_argparser` composes the two
back into the parser `ppo_ray.py` has always had.
...
#: PPO's values for the flags defined in `args_common.add_common_args`.
#: Splat into that function (``add_common_args(p, **COMMON_DEFAULTS)``)
#: to reproduce PPO's half of the shared surface exactly.
COMMON_DEFAULTS = {
    "seed": 250197,
    "dataset": "none",
    "dataset_type": str,
    ...
    "latency_inner_reps_help": (
        "Time each latency reading over a tight inner loop of N "
...
def add_ppo_args(p: argparse.ArgumentParser) -> argparse.ArgumentParser:
    """Add the PPO-only flags to `p` and return `p`."""
    # Run / logging
    ...
    # `--dataset` is shared with az; see COMMON_DEFAULTS.
    ...
    # Optimizer (`--lr` is shared with az; see COMMON_DEFAULTS)
