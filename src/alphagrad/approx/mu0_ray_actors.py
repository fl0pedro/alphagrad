...
from alphagrad.approx.env_ray import (
...
def _build_actor_state(
    ...
    def remote_eval(o_list, transforms, eval_samples):
        import random

        import ray

        if not cpu_workers:
            raise RuntimeError("No CPU workers provided for remote evaluation.")
        worker = random.choice(cpu_workers)
        return ray.get(worker.evaluate_graph.remote(o_list, transforms, eval_samples))
    ...
    env = VertexEliminationEnv.from_jaxpr(
        ...
        remote_eval_fn=remote_eval if cpu_workers else None,
    )
    ...
    def run_rollout_and_train(
        ...
        with active_mesh():
            if ds is not None and self.state["num_envs"] % num_devs == 0:
                self.state["env_states"] = jax.tree.map(
                    lambda x: jax.device_put(x, ds), self.state["env_states"]
                )
...
class CPUApproximationWorker:
    def __init__(self, args_dict: dict, variant: str, actor_id: int):
        ...
        self.target_fn = get_fn(self.args.example)
        key = jrand.PRNGKey(int(self.args.seed))
        _, args_key = jrand.split(key)
        dataset_arg = None if self.args.dataset == "none" else self.args.dataset
        use_dataset = dataset_arg is not None and self.args.example.endswith(
            "NeuralNetwork"
        )
        dataset_for_call = dataset_arg if use_dataset else None
        self.xs = get_args(self.args.example, args_key, dataset=dataset_for_call)
        self.argnums = infer_argnums(self.args.example)
        self.has_aux = False
        self.sparse = False
    ...
    def evaluate_graph(self, o_list, transforms, eval_samples):
        ...
        measure_latency = self.args.measure_latency or self.args.cmp_type == "latency"
        n_samples = 10 if measure_latency else 1
        ...
        for out_approx, out_exact in zip(out_approxs, out_exacts):
            jac_approx = out_approx[1] if self.has_aux else out_approx
            jac_exact = out_exact[1] if self.has_aux else out_exact
        ...
        return flops, bytes_accessed, peak_memory, latency_ns, cosine_sim, frob_residual
