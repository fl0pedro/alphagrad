...
def _run(args) -> int:
    ...
    # All-time record of EVERY solution ever admitted to the front — even ones
    # later dominated and pruned from the live archive. Kept for post-hoc
    # testing/reference; stores the FULL reward vector so any objective set can
    # be re-scored offline. Append-only, deduped by sequence.
    all_front_candidates: list = []
    _seen_cand: set = set()
    ...
    def _archive_add(solutions) -> None:
        ...
        for sol in solutions or []:
            ...
            added = True
            # Log this admission to the all-time candidate record (dedup by seq).
            _key = repr(seq)
            if _key not in _seen_cand:
                _seen_cand.add(_key)
                _full = vec.tolist() if hasattr(vec, "tolist") else list(vec)
                all_front_candidates.append({
                    "episode": int(_ep_box[0]),
                    "reward_vec": [float(x) for x in _full],
                    "obj": {nm: float(g[k]) for k, nm in enumerate(obj_names)},
                    "seq": seq,
                })
        if added:
    ...
    def _dump_all_candidates() -> None:
        """Persist EVERY solution ever admitted to the front (incl. ones later
        pruned) — full reward vector + sequence + the episode it entered — so
        the whole explored frontier is available for testing/reference."""
        payload = {
            "objectives": list(obj_names),
            "num_candidates": len(all_front_candidates),
            "note": (
                "every solution admitted to the non-dominated front at any "
                "point during training (including later-pruned ones); the full "
                "8-channel reward_vec is stored so any objective set can be "
                "re-scored offline."
            ),
            "candidates": all_front_candidates,
        }
        try:
            with open(all_cand_path, "w") as _f:
                _json.dump(payload, _f, indent=2)
        except Exception as _exc:
    ...
    tqdm.write(
        f"  [C-MORL] all-time front candidates ({len(all_front_candidates)}) "
        f"-> {all_cand_path}"
    )
