...
def _run_one_variant(args, variant: str) -> None:
    ...
    _arch_pts: list = []
    _arch_seqs: list = []
    # All-time record of EVERY terminal admitted to the front (incl. later-
    # pruned ones) — full reward vector + seq + episode — for post-hoc
    # testing/reference. Append-only, deduped by sequence.
        ...
        for sol in sols or []:
            ...
            if _key not in _seen_cand:
                ...
                _all_candidates.append({
                    ...
                    "obj": {n: float(g[k]) for k, n in enumerate(_obj_names)},
                    "seq": _seq,
                })
        if added:
    ...
    def _dump_all_candidates() -> None:
        """Persist EVERY terminal ever admitted to the front (incl. pruned) —
        full reward vector + sequence + entry episode — for testing/reference."""
        payload = {
            "objectives": _obj_names,
            "num_candidates": len(_all_candidates),
            "note": (
                "every terminal admitted to the non-dominated front at any "
                "point during training (including later-pruned ones); full "
                "reward_vec stored so any objective set can be re-scored."
            ),
            "candidates": _all_candidates,
        }
        try:
            os.makedirs(os.path.dirname(_all_cand_json) or ".", exist_ok=True)
            with open(_all_cand_json, "w") as _f:
                json.dump(payload, _f, indent=2)
        except Exception as _exc:
    ...
    tqdm.write(
        f"  MOGFN all-time front candidates: {len(_all_candidates)} "
        f"-> {_all_cand_json}"
    )
