...
# Not re-exported from common/__init__ since the package slim-down — import
# from its home module (the function itself is unchanged).
...
def _build_reward_weights(args) -> np.ndarray:
    ...
    LAYOUT BRIDGE: `reward_scaling` carries the DEPRECATED Ray line's
    10-channel layout (adds xla_peak_memory / bkstep_acc); the mainline
    env emits the canonical 8-vector. Map by NAME onto the env layout —
    weights for channels the env doesn't emit are dropped.
    """
    from alphagrad.approx.common import reward_scaling as _rs
    w_canonical = _rs.build_reward_weights(args)
    w = np.zeros(NUM_REWARDS, dtype=np.float32)
    for name, wi in zip(_rs.REWARD_NAMES, w_canonical):
        if name in REWARD_INDEX:
            w[REWARD_INDEX[name]] = float(wi)
    return w
...
def main():
    ...
    env = VertexEliminationEnv.from_jaxpr(
        ...
        # mu0 predates the strict TRANSFORM-DID-NOT-FIT guard: its legacy
        # pair/factor head has no live-legality oracle, so proposed rules
        # can misfit an edge. per_face routes every rule through the
        # live-masked hook (apply where legal, skip where not — the same
        # doctrine as the PPO --face-actions line) instead of dying.
        per_face=True,
    )
