...
def build_one(no_ptr, not_autoreg, total_v=6, batch_size=8, seed=0):
    ...
    agent = _build_agent(
        use_pointer, use_autoreg, args, total_v, num_rewards=2,
...
def make_loss_fn(pair_valid_mask, eps=0.2, value_weight=0.5, entropy_weight=0.05):
...
def test_variant(no_ptr, not_autoreg):
    ...
    batch = fake_batch(
        ...
        num_rewards=2, key=batch_key,
    )
    pair_valid = jnp.ones((total_v, NUM_PAIR_CHOICES))
    loss_fn = make_loss_fn(pair_valid)
