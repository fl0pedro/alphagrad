#!/bin/bash
#SBATCH --job-name=ppo_e2_smoke2
#SBATCH --time=03:00:00
#SBATCH --output=/Users/assmuth/dsnn/ppo_e2_smoke2_%j.out
...
export ALPHAGRAD_APPEND_ONLY_JAXPR=1   # Smoke-2: recompile-free append-only jaxpr policy state (AZ-parity speed upgrade)
export ALPHAGRAD_MEASURE_CACHE_CLEAR_EVERY=64
export EPISODES="${EPISODES:-10}"
export MAX_SUBSTEPS="${MAX_SUBSTEPS:-1}"
export CAMP="${CAMP:-campaign_ppo_e2_smoke2}"
...
NAME="ppo_e2_smoke2${VARIANT:+_$VARIANT}$([ "$VALUE_NORM" = popart ] && echo _popart)_s${SEED}_ss${MAX_SUBSTEPS}"
