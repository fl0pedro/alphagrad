...
export ALPHAGRAD_REWARD_CHANNELS="cosine_sim:1.0,latency_ns:1.0,xla_peak_memory:1.0"
export ALPHAGRAD_MEASURE_CACHE_CLEAR_EVERY=64
export EPISODES="${EPISODES:-25}"
export MAX_SUBSTEPS="${MAX_SUBSTEPS:-1}"
export CAMP="${CAMP:-campaign_ppo_e2_full}"
...
export ALPHAGRAD_ACC_PROXY=cosine  # E2: quality=cosine (aligned to AZ), NOT bkstep
export ALPHAGRAD_BKSTEP=0  # E2: bkstep probe OFF (cosine is the quality channel; saves ~80 train calls/terminal)
...
NAME="ppo_e2_full${VARIANT:+_$VARIANT}$([ "$VALUE_NORM" = popart ] && echo _popart)_s${SEED}_ss${MAX_SUBSTEPS}"
